...
- (void)testReceiveGNPAfterDestroy
{
        ...
        if (response) {
            ...
            [_gomClient registerGOMObserverForPath:ATTRIBUTE_2_1_PATH options:nil clientCallback:^(NSDictionary *response) {
                ...
                if ([response[@"event_type"] isEqualToString:@"initial"]) {
                    ...
                    // trigger GNP by updating attribute
                    [_gomClient destroy:ATTRIBUTE_2_1_PATH completionBlock:nil];
    ...
    // check response from update
    XCTAssertTrue([_destroyResponse[@"path"] isEqualToString:ATTRIBUTE_2_1_PATH], @"path should be equal to the reference value.");
